  ...
  if (recipe && recipe.ingredients && Array.isArray(recipe.ingredients)) {
    recipe.ingredients.forEach((item) => {
      ...
      if (ing) {
        ...
        const isLiquid =
          ing.type !== 'other' &&
          (!ing.unit || ing.unit === 'ml' || ing.unit === 'oz');
...
const SingleItemScreen = ({ ingredients, searchTerm }) => {
  const singles = ingredients.filter(
    (i) =>
      i.type === 'alcohol' &&
      i.isSingle !== false &&
      (safeString(i.nameZh).includes(searchTerm) ||
        safeString(i.nameEn).toLowerCase().includes(searchTerm.toLowerCase()))
  );
...
const FeaturedSectionScreen = ({
  ...
  const handleAddSection = () => {
    ...
    setSections([
      ...sections,
      { id: generateId(), title: newSectionTitle, subgroups: [] },
    ]);
    setNewSectionTitle('');
    ...
    const updatedSections = sections.map((s) => {
      if (s.id === sectionId) {
        return {
          ...
          subgroups: [
            ...s.subgroups,
            { id: generateId(), title: newSubgroupTitle, recipeIds: [] },
          ],
        };
      }
      return s;
      ...
      if (s.id === sectionId) {
        const updatedSubgroups = s.subgroups.map((sub) => {
          if (sub.id === subgroupId) {
            return {
              ...sub,
              recipeIds: sub.recipeIds.filter((id) => id !== recipeId),
            };
          }
          return sub;
        });
  ...
  if (!activeSectionId) {
    return (
      <div className="w-full pb-24 animate-fade-in">
        <div className="sticky top-0 bg-slate-950/95 backdrop-blur z-20 border-b border-slate-800 shadow-md px-4 pt-safe pb-3">
  ...
  return (
    <div className="w-full pb-24 animate-slide-up">
      <div className="sticky top-0 bg-slate-950/95 backdrop-blur z-20 border-b border-slate-800 shadow-md px-4 pt-safe pb-3">
          ...
          {activeSection.subgroups.map((subgroup) => (
            <div key={subgroup.id} className="space-y-3 relative">
              <div className="flex items-center justify-between border-l-4 border-amber-500 pl-3 py-1 bg-slate-900/50 rounded-r-lg pr-2">
                <h3 className="text-lg text-slate-200 font-bold">
                  {subgroup.title}
                </h3>
                <div className="flex gap-2">
                  <button
                    onClick={() =>
                      openRecipePicker(activeSection.id, subgroup.id)
                    }
                    className="text-emerald-400 text-xs bg-emerald-900/10 px-3 py-1.5 rounded flex items-center gap-1 border border-emerald-500/30 hover:bg-emerald-900/30 transition-colors"
                  >
                    <Plus size={12} /> 加入酒譜
                  </button>
                  {isEditing && (
                  ...
                  subgroup.recipeIds.map((rId) => {
                    ...
                    return (
                      <div
                        ...
                        className="flex bg-slate-800 rounded-xl overflow-hidden border border-slate-800 hover:border-slate-600 active:scale-[0.99] transition-all relative group shadow-md"
                      >
                        <div className="w-20 h-20 bg-slate-900 shrink-0">
                          {recipe.image ? (
                            ...
                            <div className="flex items-center justify-center h-full">
                              <Wine size={20} className="text-slate-700" />
                            </div>
                          )}
                        </div>
                        <div className="p-3 flex-1 flex flex-col justify-center min-w-0">
                          <div className="font-bold text-slate-200 text-base leading-tight truncate">
                            {safeString(recipe.nameZh)}
                          </div>
                          <div className="text-slate-500 text-xs font-serif italic truncate">
                            {safeString(recipe.nameEn)}
                          </div>
                          <div className="flex gap-2 mt-1">
                            <span className="text-[10px] text-amber-500 bg-amber-900/20 px-1.5 rounded border border-amber-900/50">
                              ${stats.price}
                            </span>
                          </div>
                        </div>
                        {isEditing && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              removeRecipeFromSubgroup(
                                activeSection.id,
                                subgroup.id,
                                rId
                              );
                            }}
                            className="absolute right-0 top-0 bottom-0 bg-rose-900/90 text-white w-12 flex items-center justify-center animate-fade-in hover:bg-rose-600 z-10"
                          >
                            <X size={20} />
                          </button>
                        )}
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1">
              {recipes
                ...
                .map((r) => (
                  <button
                    key={r.id}
                    onClick={() => addRecipeToSubgroup(r.id)}
                    className="w-full text-left p-3 hover:bg-slate-800 rounded flex items-center gap-3 border-b border-slate-800/30"
                  >
                    <div className="w-8 h-8 bg-slate-800 rounded overflow-hidden shrink-0">
                      {r.image ? (
                        ...
                        <Wine size={16} className="m-auto text-slate-600" />
                      )}
                    </div>
                        ...
                        <button
                          onClick={(e) => {
                            ...
                            const q = encodeURIComponent(
                              `${recipe.nameZh} ${recipe.nameEn} 雞尾酒 調酒`
                            );
                            window.open(
              ...
              <div>
                ...
                <div className="text-slate-500 text-xs">
                  ...
                  {ing.type === 'alcohol' && ing.subType && (
                    <span className="ml-2 text-[10px] bg-slate-700 px-1.5 py-0.5 rounded text-slate-400">
              ...
              <div className="mt-4 pt-4 border-t border-slate-800">
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setItem({ ...item, isSingle: e.target.checked })
                    }
                  />
                  <span className="text-sm text-slate-300 group-hover:text-white">
                    列入單品菜單 (Available as Single)
                  </span>
                </label>
                <p className="text-[10px] text-slate-500 mt-1 ml-8">
                  取消勾選後，此材料將不會出現在「純飲/單品」頁面中。
                </p>
              </div>
            )}
            {item.id && (
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2 mb-4">
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-sm font-bold transition-colors border ${
                      item.type === 'signature'
                        ? 'bg-amber-900/50 border-amber-500 text-amber-500'
                        : 'bg-transparent border-slate-700 text-slate-500 hover:border-slate-500'
                    }`}
                  >
                    特調 Signature
...
function MainAppContent() {
  ...
  const [activeTab, setActiveTab] = useState(
    () => localStorage.getItem('bar_active_tab_v3') || 'recipes'
  );
        ...
        if (mode === 'overwrite') {
          ...
          setIngredients((prev) => {
            const newItems = (data.ingredients || []).filter((newI) => {
              ...
              const nameExists = prev.some(
                (oldI) =>
                  oldI.nameZh &&
                  newI.nameZh &&
                  oldI.nameZh.replace(/\s+/g, '') ===
                    newI.nameZh.replace(/\s+/g, '')
              );
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-amber-500/30 w-full overflow-x-hidden">
      ...
      {activeTab === 'recipes' && (
        <RecipeListScreen
          ...
          availableBases={BASE_SPIRITS}
          // Removed sections prop since Featured is now standalone
        />
      )}
      ...
      {activeTab === 'tools' && (
        <div className="p-6 text-center space-y-6 pt-20 w-full">
          ...
          <div className="space-y-3">
            <button
              onClick={() => {
                const data = JSON.stringify({ ingredients, recipes, sections });
